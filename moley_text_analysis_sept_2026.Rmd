...
I am focusing on the time period of Moley's work in the Los Angeles Times and Newsweek, from **1937-1969**, with **4063 articles**  analyzed. 

There are 4063 articles in the index of Moley Newsweek and LAT columns.
The earliest year of publication is 1937, and the latest is 1969.
The average year of publication is 1953, with the majority of articles written in 1951.

The article text has 4049 articles. That's less than the 4063 entries in the article index. The difference was due to about 14 failed AI scans.
The article text dataframe is huge, with 400170 rows and 9 columns.
...
#write_csv(text_combo, "./data/lat_newsweek_text_combo_9_23.csv")
#write_csv(index_combo, "./data/lat_newsweek_INDEX_combo_9_23.csv")
...
## Statistics about the combo index 
```{r}
nrows <- nrow(index_combo)
ncols <- ncol(index_combo)

mean_year <- round(mean(index_combo$year, na.rm = TRUE),0)

min_year <- min(index_combo$year)

max_year <- max(index_combo$year)
...
glue::glue("There are {nrows} articles in the index of Moley Newsweek and LAT columns.");
...
```{r echo = F}
nrows1 <- nrow(text_combo)
ncols1 <- ncol(text_combo)
...
#Sandwich it onto a simple ggplot
  ...
  labs(title = "Moley Newsweek Columns By Year, 1937-1967", 
       ...
       caption = "n=4,063  articles. Graphic by Rob Wells, 9/23/2026",
       y="Count of Pages",
       x="Year")
...
# Bigrams
```{r}
bigrams <- text_combo %>% mutate(sentence= str_squish(sentence)) |> 
  ...
  mutate(text = str_replace_all(text, "copyright owner", "")) %>%
  mutate(text = str_replace_all(text, "reproduction prohibited", "")) %>%
...
ggplot(top_20_bigrams, aes(n, reorder(bigram, n), fill=n)) +
  ...
  labs(title = "Raymond Moley Common Phrases, 1937-1967",
       subtitle = "Analysis of 4,063 Newsweek, LAT columns",
       x = "Count",
       y = "Bigrams",
       caption = "Source: Newsweek. Graphic by Rob Wells and Bridget Lang, 9-24-2026")
```
...
text_combo %>%
  ...
  mutate(percentage = formattable::percent(count / 4063,0)) %>%
...
print(paste("The search shows Moley used the term 'statism' in", nrow(statism), "instances in various Newsweek and LAT columns."))
```
...
```{r}
# Create corpus
my_corpus <- corpus(text_combo, text_field = "sentence")
# Tokenize 
...
# Map docvars directly using document identifiers
# Gemini Flash used to extract and place pubtitle into the output
kwic_reuther <- reuther %>%
  mutate(
    pubtitle = docvars(my_corpus, "pubtitle")[match(docname, docnames(my_corpus))],
    year     = docvars(my_corpus, "year")[match(docname, docnames(my_corpus))],
    filename = docvars(my_corpus, "filename")[match(docname, docnames(my_corpus))]
  ) %>%
  select(docname, filename, year, pubtitle, pre, keyword, post) %>%
  arrange(year)
...
print(paste("Moley referred to UAW President Walter Reuther in",nrow(wr), "articles Newsweek and the LAT. Of that, 110 references to Reuther were in Newsweek columns"))
```
...
kwic_reuther |> 
  ...
  ungroup() |> 
  ggplot(aes(x = year, y = n_articles, fill = pubtitle)) +
  geom_col() +
  scale_x_continuous(breaks = seq(1942, 1966, by = 2)) +
  labs(
    title = "Walter Reuther Mentions in Moley Coverage",
    subtitle = "Number of Unique Newsweek, LAT Columns Mentioning Reuther per Year",
    y = "Article Count", 
    x = NULL,
    caption = "n=4,063 Newsweek, LAT articles, Graphic by Rob Wells"
  ) +
  theme_minimal() +
...
###Goldwater
```{r}
goldwater_articles2 <- text_combo %>%
  filter(!is.na(sentence)) %>%
  group_by(filename, year, date) %>%
...
###Roosevelt
```{r}
roosevelt_articles2 <- text_combo %>%
  filter(!is.na(sentence)) %>%
  group_by(filename, year, date) %>%
...
###Eisenhower
```{r}
eisenhower_articles2 <- text_combo %>%
  filter(!is.na(sentence)) %>%
  group_by(filename, year, date) %>%
...
# Let's also check the intermediate steps to debug:
# First check if we're finding any Nixon mentions at all
eisenhower_mentions <- text_combo %>%
  filter(!is.na(sentence)) %>%
...
## Count articles involving politicians
```{r}
politicians |> 
  group_by(politician) |> 
  distinct(filename) |> 
...
```{r echo = F}
nixon_articles <- text_combo %>%
  filter(str_detect(sentence, "Nixon")) %>%
  select(filename, year) %>%
  distinct(filename, year)


goldwater_articles <- text_combo %>%
  filter(str_detect(sentence, "Goldwater")) %>%
  select(filename, year) %>%
  distinct(filename, year)

roosevelt_articles <- text_combo %>%
  filter(str_detect(sentence, "Roosevelt")) %>%
  select(filename, year) %>%
  distinct(filename, year)

dewey_articles <- text_combo %>%
  filter(str_detect(sentence, "Dewey")) %>%
  select(filename, year) %>%
  distinct(filename, year)
...
```{r}
goldwater_index <- text_combo %>%
  filter(!is.na(sentence)) %>%
  group_by(filename, year, date) %>%
  summarize(mentions = sum(str_detect(sentence, "Goldwater"), na.rm = TRUE)) %>%
  filter(mentions >= 3)

goldwater_major_articles <- text_combo |> 
  inner_join(goldwater_index, by = c("filename", "date", "year"))

goldwater_major_index_combo <- goldwater_major_articles |> 
  select(year, date, title, abstract, filename, pubtitle) |> 
  distinct(filename, .keep_all = TRUE) |> 
  arrange(date)
  


#write_csv(goldwater_major_index_combo, "goldwater_major_index_combo.csv")

```
# Print out major Goldwater articles to read
```{r}
...
for (i in 1:nrow(major)) {
  ...
  writeLines(paste0("Publication: ", row_data$pubtitle), file_conn)
...
goldwater_timeline <- goldwater_major_index_combo |> 
  rename(subject = abstract) |> 
  mutate(author = "Moley article") |> 
  full_join(letters) |> 
 subset(select = -c(year)) |> 
  select(date, author, pubtitle, subject, recipient, everything()) |> 
  arrange(date)
...
maxqda_merged <- maxqda_binary |> 
  inner_join(index_combo, by=c("date_new"="date"))
...
maxqda_articles <- maxqda_binary |> 
  inner_join(text_combo, by=c("date_new" = "date", "year" = "year"))
...
annotated_text_moley <- udpipe_annotate(udmodel, x = text_combo$sentence, doc_id = text_combo$filename)
...
entities_moley <- spacy_parse(
  x = text_combo$sentence,  # Your text column
...
unigrams <- text_combo %>% mutate(sentence= str_squish(sentence)) |> 
  mutate(text = tolower(sentence)) |>  
  mutate(text = gsub("\\d+", "", text)) |>
  mutate(text = str_replace_all(text, "raymond", "")) %>% 
  mutate(text = str_replace_all(text, "newsweek", "")) %>% 
